...
namespace UL.Aria.Service.Performance.Configuration
{
	internal class UnityInstanceProvider : Disposable
	{
		...
		public static UnityInstanceProvider Create()
		{
			...
			container.RegisterInstance<IPerformanceConfigurationSource>(new PerformanceConfigurationSource())
				.RegisterType<IPerformanceManagerFactory, PerformanceManagerFactory>(new ContainerControlledLifetimeManager())
		...
		internal static void RegisterManagers(IUnityContainer container)
		{
			...
			builder.RegisterAssemblyTypes(Assembly.GetExecutingAssembly())
				...
				.Where(x => x.GetInterface(typeof (IPerformanceManager).FullName) != null)
		...
		{
			var builder = new ContainerBuilder(container);
			builder.RegisterAssemblyTypes(Assembly.GetAssembly(typeof (ICertificateService)))
				.IgnoreSystemInterfaces()
				.SetActivatableClassifiersOnly()
				.AsImplementedInterfaces();
		}
