...
.PARAMETER ServerRole
	Type of server (Portal, Services SPWFE, SPAPP, SQL, AD, Tools)
...
{
	$serverRoles = @("AD","SQL", "SPWFE", "SPAPP", "Portal", "Services", "OnPrem", "Tool", "AD_NTH", "Quorum")
...
function Get-DeploymentTier([Parameter(Mandatory=$true)][String]$ServerName, [Parameter(Mandatory=$true)][object]$EnvironmentConfiguration)
{
        ...
        if ($Environment.ToolsServers -ne $null)
        {
            foreach ($ToolsServer in @($Environment.ToolsServers))
            {
                Write-Debug "Server role is Tool"
                return "Tool"
            }
        }
...
.DESCRIPTION
	If an environment name isn't provided, one is inferred from the computername and the environment.xml file.
#>
...
.DESCRIPTION
	If an environment configuration file name isn't provided, one is inferred from the computername or the EnvironmentName and the environment.xml file.
#>
...
.DESCRIPTION
	If deployment tier isn't provided, one is inferred from the computername and the environment.xml file.
#>
...
#region Configuration_Data_Functions
 ...
 function Get-EnvironmentConfiguration([Parameter(Mandatory=$true)][string]$EnvironmentConfigurationFileName, [Parameter(Mandatory=$true)][string]$EnvironmentName, [Parameter(Mandatory=$false)][String]$XMLDirectory)
 {
	...
	$ToolsServers = @($Environment.ToolsServers.Split())
        ...
        $Certs = @($Environment.Certificates.Certificate)
        ...
        #Redis Cache
        $Domain = $null;
        if(![string]::IsNullOrWhiteSpace($EnvironmentConfigurations.Domain))
        {
            $Domain = $EnvironmentConfigurations.Domain;
        }
        else
        {
            $compFullName = [System.Net.Dns]::GetHostByName(($env:computerName)).HostName;
            $pos = $compFullName.IndexOf(".") + 1;
            $Domain = $compFullName.Substring($pos, $compFullName.Length - $pos);
        }   
    
      
        $RedisCacheConfig = $Environment.RedisAzureSettings.SelectSingleNode("AzurePortalMachine[@Domain=""" + $Domain + """]")
	    ...
	    ToolsServers = @($ToolsServers)
            ...
            #Azure
            #ULAriaWebPortalWebSiteName = $EnvironmentConfigurations.ULAriaWebPortalWebSiteName
            ULAriaServiceHostServiceName = $EnvironmentConfigurations.ULAriaServiceHostServiceName
            ServiceHostInternalLoadBalancerIP = $EnvironmentConfigurations.ServiceHostInternalLoadBalancerIP
	    XmlSourceRef = $Environment; 
        }
