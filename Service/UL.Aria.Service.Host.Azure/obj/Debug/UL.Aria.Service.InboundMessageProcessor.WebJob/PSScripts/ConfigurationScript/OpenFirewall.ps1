...
{
    Write-Host ($nl + "Enabling Inbound Firewall Rule " + $RuleName + " on " + $Profile)
...
function CreateAllPortsRules()
{
    ...
    Write-Debug ($nl + "Creating All Ports rules - UDP")
...
function EnableFirewallRulesForOtherSqlServices()
{
		...
		#Write-Host ($nl + "Enabling Conventional SQL Server Service Broker port 4022")
		#netsh advfirewall firewall add rule name="SQL Service Broker" dir=in action=allow protocol=TCP localport=4022

		#Write-Host ($nl + "Enabling Transact SQL/RPC port 135")
		#netsh advfirewall firewall add rule name="SQL Debugger/RPC" dir=in action=allow protocol=TCP localport=135

		#Write-Host ($nl + "=========  Analysis Services Ports  ==============")
		#Write-Host ($nl + "Enabling SSAS Default Instance port 2383")
		#netsh advfirewall firewall add rule name="Analysis Services" dir=in action=allow protocol=TCP localport=2383

		#Write-Host ($nl + "Enabling SQL Server Browser Service port 2382")
		#netsh advfirewall firewall add rule name="SQL Browser" dir=in action=allow protocol=TCP localport=2382

		#Write-Host ($nl + "=========  Misc Applications  ==============")
		#Write-Host ($nl + "Enabling HTTP port 80")
		#netsh advfirewall firewall add rule name="HTTP" dir=in action=allow protocol=TCP localport=80
		#Write-Host ($nl + "Enabling SSL port 443")
		#netsh advfirewall firewall add rule name="SSL" dir=in action=allow protocol=TCP localport=443
		#Write-Host ($nl + "Enabling port for SQL Server Browser Service's 'Browse' Button")
		#netsh advfirewall firewall add rule name="SQL Browser" dir=in action=allow protocol=UDP localport=1434
		#Write-Host ($nl + "Allowing multicast broadcast response on UDP (Browser Service Enumerations OK)")
...
)
{
	...
	if ($ServerRole -eq "SQL")
	{
		...
		Write-Host ($nl + "=========  SQL Server Ports  ===================")
		Write-Host ($nl + "Enabling SQLServer default instance port 1433")
		netsh advfirewall firewall add rule name="SQL Server" dir=in action=allow protocol=TCP localport=1433
		
		Write-Host ($nl + "Enabling Dedicated Admin Connection port 1434")
		netsh advfirewall firewall add rule name="SQL Admin Connection" dir=in action=allow protocol=TCP localport=1434

		Write-Host ($nl + "Enabling AlwaysOn Listener Internal Load Balancer Probe Port")
		netsh advfirewall firewall add rule name="AriaSP_AG_Listener Probe Port" dir=in action=allow protocol=TCP localport=59999

		#Write-Host ($nl + "Enabling UL_Aria instance port 1435")
